...
void GuiController::openTrack()
{
    QString filename=QFileDialog::getOpenFileName(nullptr,tr("Open track file..."),QString(),tr("Track Files (*.xml)"));
    if(filename.isEmpty())
        return;
    if(!(m_track=m_simulation_runner->loadTrack(filename)))
        QMessageBox::warning(nullptr,tr("Track loading error"),tr("Can't load track file."));
    connect(m_renderer->getQuickWindow(),&QQuickWindow::beforeRendering,this,&GuiController::initScene,Qt::DirectConnection);
}

void GuiController::openAlgorithm()
{
    QString filename=QFileDialog::getOpenFileName(nullptr,tr("Open algorithm file..."),QString(),tr("Lua Script Files (*.lua)"));
    if(filename.isEmpty())
        return;
    if(!m_simulation_runner->loadAlgorithmFile(filename))
        QMessageBox::warning(nullptr,tr("Lua script loading error"),tr("Can't load algorithm file."));
}
...
void GuiController::runSimulation()
{
    ...
    connect(m_renderer->getQuickWindow(),&QQuickWindow::beforeRendering,m_car,&Car3D::update,Qt::DirectConnection);
...
{
    disconnect(m_renderer->getQuickWindow(),&QQuickWindow::beforeRendering,this,&GuiController::initScene);
    if(m_car)
        delete m_car;
